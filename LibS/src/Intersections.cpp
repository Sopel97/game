...
{
    if(a.topLeft.x >= b.bottomRight.x) return false;
    if(a.topLeft.y >= b.bottomRight.y) return false;
    if(a.bottomRight.x <= b.topLeft.x) return false;
    if(a.bottomRight.y <= b.topLeft.y) return false;
    return true;
}
template <class T>
bool Intersections::intersection(const Rectangle<T>& a, const Circle<T>& b)
{
    double closestX = clamp(b.center.x, a.topLeft.x, a.bottomRight.x);
    double closestY = clamp(b.center.y, a.topLeft.y, a.bottomRight.y);
...
bool Intersections::intersection(const Cuboid<T>& a, const Cuboid<T>& b)
{
    if(a.topLeft.x >= b.bottomRight.x) return false;
    if(a.topLeft.y >= b.bottomRight.y) return false;
    if(a.topLeft.z >= b.bottomRight.z) return false;
    if(a.bottomRight.x <= b.topLeft.x) return false;
    if(a.bottomRight.y <= b.topLeft.y) return false;
    if(a.bottomRight.z <= b.topLeft.z) return false;
    return true;
}
...
{
    return (b.x > a.topLeft.x && b.x < a.bottomRight.x && b.y > a.topLeft.y && b.y < a.bottomRight.y);
}
template <class T>
