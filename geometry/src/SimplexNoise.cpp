...
{
    auto grad = [](const int hash, const T x) //why not an array like in 3d and 4d?
    {
        int h = hash & 15;
...
T SimplexNoise<T>::rawNoise2(const T x, const T y) const
{
    ...
    T F2 = 0.5 * (sqrt(3.0) - 1.0);
    // Hairy factor for 2D
    T s = (x + y) * F2;
    int i = fastFloor( x + s );
    int j = fastFloor( y + s );

    T G2 = (3.0 - sqrt(3.0)) / 6.0;
    T t = (i + j) * G2;
...
{
    return octaveNoise1(x, octaves, persistence, scale) * (higherBound - lowerBound) / 2. + (higherBound + lowerBound) / 2.;
}
template <class T>
...
{
    return octaveNoise2(x, y, octaves, persistence, scale) * (higherBound - lowerBound) / 2. + (higherBound + lowerBound) / 2.;
}
template <class T>
...
{
    return octaveNoise3(x, y, z, octaves, persistence, scale) * (higherBound - lowerBound) / 2. + (higherBound + lowerBound) / 2.;
}
template <class T>
...
{
    return octaveNoise4(x, y, z, w, octaves, persistence, scale) * (higherBound - lowerBound) / 2. + (higherBound + lowerBound) / 2.;
}
